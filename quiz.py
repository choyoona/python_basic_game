...
character_x_pos = random.randrange(480) # (screen_width / 2) - (character_width / 2) 
character_y_pos = screen_height - character_height  # 화면 세로 크기 가장 아래에 해당되는 곳에 위치
character_speed = 1

ddong = pygame.image.load("/Users/yoona/Documents/Python/pygame_bas/enemy.png")
ddong_size = ddong.get_rect().size # 이미지의 크기를 구해옴
ddong_width = ddong_size[0]  # 캐릭터의 가로 크기
ddong_height = ddong_size[1] # 캐릭터의 세로 크기
ddong_x_pos = random.randrange(480)  # random.randint(0, screen_width - ddong_width)
ddong_y_pos = 0 
ddong_speed = 10
...
while running :
    dt = clock.tick(30) # 게임 화면의 초당 프레임 수를 결정 
    ...
    if ddong_y_pos > screen_height:
        ddong_y_pos = 0
        ddong_x_pos = random.randrange(480)
    ...
    enemy_rect = ddong.get_rect()
    enemy_rect.left = ddong_x_pos
    enemy_rect.top = ddong_y_pos
